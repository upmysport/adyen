...
if File.exist?(API_SPEC_INITIALIZER)
  ...
  describe Adyen::API, "with an actual remote connection" do
    ...
    context 'when request is submitted to recurring service' do
      ...
      before(:all) { perform_payment_request }
    ...
    context 'when request is submitted to payout service' do
      let(:stored_details_response) do
        Adyen::API.store_bank_detail(
      ...
      it "store bank details" do
        stored_details_response.should be_success
      end

      it "submits a payout" do
        response = Adyen::API.submit_payout(
          "PayoutPayment-0001",
          {
            :currency => "EUR",
            :value => 100
          ...
          stored_details_response.recurring_detail_reference
        )
